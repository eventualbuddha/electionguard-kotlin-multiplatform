...
// is prepared to deal with a suspending function. That's ultimately why we need to
// require that productionGroup() is a suspend function, across every platform we support.

// **Minimal interface to GMP-WASM**: GMP-WASM defines a bunch of wrappers around the
// GMP primitives, but these take advantage of things like TypeScript's union types,
// so they don't map as nicely as we'd like to the Kotlin type system. The solution,
// for now, is to just export the interface to the low-level C functions, and do
// all of the work ourselves, in our BigInteger class, which is fairly minimal relative
// to the more general-purpose needs of other library users.
